...
do
  ...
  res=$(../vampire_dbg_master_6829 --mode $mode -sa otter --input_syntax tptp -t 5m -m 8000 --show_preprocessing false -p off -tstat on $path)
  #echo "$res"
  mkdir -p "$mode/$fragment"
  echo "$res" > "$mode/$fragment/$problem.out"
  echo End of $problem
done<$inFile
